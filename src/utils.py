...
sys.path.append("\\data\\wjh\\graduate\\New\\")
import json_lines
import numpy as np
from tqdm import tqdm
from transformers import BasicTokenizer
ROOT_DIR = os.path.abspath("\\data\\wjh\\graduate\\New")
...
def read_RTE(input_file, is_training):
    ...
    examples, chunks = [], []
    # 参考 https:\\\\blog. .net\\sinat_29675423\\article\\details\\87972498
...
def read_SICK(input_file, is_training):
    ...
    if not is_training:
        ...
        dir = '\\data\\wjh\\graduate\\NEW_SICK\\SICK.txt'
        ...
        dir = '\\data\\wjh\\graduate\\NEW_SICK\\sick_new_train.tsv'
        df = pd.read_csv(dir, sep = '\t', header=0, quoting=3)
        for index, row in df.iterrows():
            print(index)
...
def read_MRPC(input_file, is_training):
    ...
    suffix = suffix[0] if is_training else suffix[-1] 
    # dir_ = os.path.join(input_file, suffix)
    # dir_ = "glue_data\\MRPC\\msr_paraphrase_test.txt"
    dir_ = "glue_data\\MRPC\\msr_paraphrase_test.txt"
    examples, chunks = [], []
    index = 0
...
#     '''二分类'''
#     output = "glue_data\\MRPC\\train.tsv"
#     df = pd.read_csv(output, sep = '\t')
#     # df = pd.read_csv(output, sep = '\t' ,chunksize=10) #防止内存一处 每10行方式进行读取
#
#     examples, chunks = [], []
#     index = 0
#     for i, row in df.iterrows():
