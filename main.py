...
def get_dataloader(args, features, features_chunks, labels, batch_size,  is_training):
    ...
    dataloader = DataLoader(dataset,
                            batch_size=batch_size, #批次中的样本数量
                            shuffle=is_training, #打乱数据集中的样本顺序
                            num_workers=4 #紫禁城数量
                            )
    return dataloader
...
def test(args, model, device, tokenizer, albert_tokenizer):
    ...
    for i, (feature_batch, feature_chunk_batch, label_batch) in enumerate(tqdm(test_dataloader)):
        with torch.no_grad():
            ...
            # Assuming feature_batch contains input_ids, attention_mask, and token_type_ids
            input_ids = feature_batch['input_ids']
            attention_mask = feature_batch['attention_mask']
            token_type_ids = feature_batch['token_type_ids']
            # loss, logits = model(feature_batch, feature_chunk_batch, label_batch , None, labels)
            # loss, kl, logits = model(feature_batch, feature_chunk_batch, label_batch , None, labels)
            loss, kl, logits = model(input_ids, attention_mask, token_type_ids, None, label_batch)
            ...
            label_ids = label_batch.to('cpu').numpy()
            print(flat_accuracy(logits, label_ids))
            print(flat_f1(logits, label_ids))
    ...
    df = pd.DataFrame(res)
    df.to_csv('data\\wjh\\graduate\\data\\submit\\test{}.tsv'.format(args["name"]), sep = '\t', index=False, header=True)
...
def evaluate(args, model, device, tokenizer, albert_tokenizer):
    if args["read_data"] or args["name"] == "MNLIMM":
        ...
        pickle_file = os.path.join(args["pickle_folder"], "evaluate_kk_features_{}.pkl".format(args["name"]))
        eval_data = {
...
def run(args):
    ...
    if args["read_data"]:
        ...
        train_features, train_chunks_features, labels = convert_examples_to_features(args, train_examples, train_chunks, albert_tokenizer, tokenizer, args["max_len"], is_training=True)
        if not os.path.exists(args["pickle_folder"]):
            os.makedirs(args["pickle_folder"])
        pickle_file = os.path.join(args["pickle_folder"], "train_KK_features_{}.pkl".format(name))
        train_data = {
            "train_features": train_features,  # input_ids attention_mask label
    ...
    scheduler = get_cosine_schedule_with_warmup(optimizer, num_warmup_steps=int(t_total) * args["warm_up_rate"], num_training_steps=t_total)
    ...
    best_eval_loss = float('inf')  # 记录最佳验证集损失，初始化为无穷大
    ...
    for iter in range(args["num_train_epochs"]):
        ...
        eval_results = evaluate(args, model, device, albert_tokenizer, tokenizer)
        eval_acc = eval_results["acc"]
        eval_loss = eval_results["val_loss"]

        if eval_acc > best_eval_acc:
            best_eval_acc = eval_acc
            save_model(args, model)
            print(f"Saving new best model with accuracy: {best_eval_acc:.4f}")
        print("epoch:", iter)
